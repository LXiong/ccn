...
#if defined(UMTS_LIB) || defined(MUOS_LIB)
...
#ifdef JVMF_LIB
#include "app_jvmf_platform.h"
#include "app_jvmf_server.h"
#include "vmf-api.h"
#include <string>
#include <sstream>
#include "WallClock.h"
...
//takahashi
#include "ccn/ccn.h"
#include "ccn/nodeData.h"
IdToNodePtrMap* GlobalNodeData::nodeHash;
    ...
    const NodeInput *nodeInput)
{
    ...
    if (retVal == TRUE)
    {
        for (i = 0; i < appInput.numLines; i++)
        {
            ...
            //takahashi
            else if(!strcmp(appStr, "CCN"))
            {
              char sourceString[MAX_STRING_LENGTH];
              char roleString[MAX_STRING_LENGTH];
              NodeRole    role;
              NodeAddress sourceNodeId;
              Address     sourceAddr;
              NodeAddress destNodeId;
              ...
              GlobalNodeData::nodeHash = nodeHash;
              ...
              if(!strcmp(roleString, "SERVER")) {
                 role = SERVER;
              } else if(!strcmp(roleString, "RELAY_NODE")) {
                 role = RELAY_NODE;
              } else if(!strcmp(roleString, "CLIENT")) {
                 role = CLIENT;
              } else {
                ERROR_ReportError("Undefined node role \n");
              }
              ...
              //NodeIDから送信元ノードを取得し、初期化
              node = MAPPING_GetNodePtrFromHash(nodeHash, sourceNodeId);

              if(node != NULL)
              {
                AppCCNNodeInit(node, role, nodeInput);
              }
            }
            ...
            if (strcmp(appStr, "JVMF") == 0)
            {
                BOOL isNodeId;

                std::string messageFormat;
                bool messageFormatSet(false);

                char platformIdIn[MAX_STRING_LENGTH];
                NodeAddress platformId;
                Address platformAddr;

                unsigned short srcPort = JVMF_CLIENT_SOURCE_PORT;

                char dstAddrIn[MAX_STRING_LENGTH];
                Address dstAddress;
                NodeId destNodeId;

                clocktype timeInterval;
                char timeIntervalIn[MAX_STRING_LENGTH];

                double distanceTraversed;

                clocktype distanceResolution = TIME_ConvertToClock("1S");
                bool distanceResolutionSet(false);

                clocktype startTime;
                char startTimeIn[MAX_STRING_LENGTH];

                clocktype endTime;
                char endTimeIn[MAX_STRING_LENGTH];

                std::string unitTypeIn;
                bool unitTypeSet(false);

                char* input = appInput.inputStrings[i];
                ...
                static const std::string usage =
                    ...
                    "  OPTIONAL:\n"
                    "    -SRCPORT <source port>\n"
                    "    -RESOLUTION <distance interval resolution timer>\n"
                    "    -UNIT <Unit type (Aircraft|Ground)>\n"
                    "    -FORMAT <format type> (None, RIFLEMAN, GMR, C2, JNE)\n";
                ...
                if (sscanf(input,
                           ...
                           endTimeIn) != 6)
                {
                    std::string str = "JVMF: Required parameters are not configured correctly.";
                    str += usage;
                    ERROR_ReportError(str.c_str());
                }
                ...
                while (ss.good()) {
                    ...
                    if (param == "FORMAT") {
                        ...
                        std::string temp;
                        ss >> temp;
                        distanceResolution = TIME_ConvertToClock(temp.c_str());
                        distanceResolutionSet = true;
                    }                    
                    else if (param == "UNIT") {
                        ss >> unitTypeIn;
                        unitTypeSet = true;
                ...
                if (MAPPING_GetNetworkType(dstAddrIn) == NETWORK_IPV6)
                {
                    dstAddress.networkType = NETWORK_IPV6;

                    IO_ParseNodeIdOrHostAddress(
                        dstAddrIn,
                        &dstAddress.interfaceAddr.ipv6,
                        &destNodeId);
                }
                else
                {
                    dstAddress.networkType = NETWORK_IPV4;

                    IO_ParseNodeIdOrHostAddress(
                        dstAddrIn,
                        &dstAddress.interfaceAddr.ipv4,
                        &isNodeId);
                }

               if (dstAddress.networkType == NETWORK_IPV4)
               {
                   if (!(GetIPv4Address(dstAddress) >= IP_MIN_MULTICAST_ADDRESS))
                   {
                        fprintf(stderr, "JVMF: destination address is not a "
                                 "multicast address\n");
                   }
               }
               else
               {
                    if (!IS_MULTIADDR6(GetIPv6Address(dstAddress)))
                    {
                        fprintf(stderr, "JVMF: destination address is not a "
                                 "multicast address\n");
                    }
                ...
                IO_AppParseSourceString(
                        ...
                        &platformId,
                        &platformAddr,
                        platformAddr.networkType);

                timeInterval = TIME_ConvertToClock(timeIntervalIn);
                startTime = TIME_ConvertToClock(startTimeIn);
                endTime = TIME_ConvertToClock(endTimeIn);

                // Initialize client
                node = MAPPING_GetNodePtrFromHash(nodeHash, platformId);
                if (node != NULL)
                {
                    AppJVmfPlatform::InitDataModel(node, nodeInput);
                    ...
                    {
                        AppJVmfPlatform::Initialize(node,
                                                    platformAddr,
                                                    srcPort,
                                                    dstAddress,
                                                    timeInterval,
                                                    ...
                                                    APP_DEFAULT_TOS,
                                                    unitTypeIn.c_str(),
                                                    VMF::Wrapper::parseSerializationType(messageFormat.c_str())
                                                    #ifdef ADDON_DB
                                                    ,
                                                    appName
                                                    #endif //ADDON_DB
                    ...
                    if (platformAddr.networkType != dstAddress.networkType)
                    {
                        char err[MAX_STRING_LENGTH];

                        sprintf(err,
                        "JVMF: At node %d, Source and Destination addresses"
                        " version mismatch inside %s\n",
                        node->nodeId,
                        appInput.inputStrings[i]);
                ...
                } // end of source node is not NULL
            }            
#endif //end of JVMF
            else
...
#ifdef MUOS_LIB
                        // In the case of MUOS, it has its own alias override
                        SipData* sip = (SipData*) multimedia->sigPtr;
                        strcpy(sourceAliasAddr, sip->SipGetOwnAliasAddr());
#endif
                    }
...
#else // UMTS_LIB || MUOS_LIB
                 ERROR_ReportMissingLibrary(appStr, "UMTS or MUOS Libraries");
#endif // UMTS_LIB || MUOS_LIB
            } // end of Umts Call
...
void APP_ProcessEvent(Node *node, Message *msg)
{
    ...
    switch(protocolType)
    {
        ...
        case APP_CCN_CLIENT:
        {
            AppLayerCCNClient(node, msg);
            break;
        }
        case APP_CCN_HOST:
        {
            AppLayerCCNHost(node, msg);
            break;
        }
        case APP_CBR_CLIENT:
        {
...
#ifdef JVMF_LIB
        // VMF
        case APP_JVMF_PLATFORM:
        {
            AppJVmfPlatform::HandleAppLayer(node, msg);
            break;
        }
        case APP_JVMF_SERVER:
        {
            AppJVmfServer::HandleAppLayer(node, msg);
            break;
        }
#endif // END JVMF_LIB
   ...
   // DHCP
    ...
    for (appList = node->appData.appPtr; appList != NULL; appList = nextApp)
    {
        switch (appList->appType)
        {
            ...
            case APP_CCN_CLIENT:
            {
              AppCCNClientFinalize(node, appList);
              break;
            }
            case APP_CCN_HOST:
            {
              AppCCNHostFinalize(node, appList);
              break;
            }
            case APP_CBR_CLIENT:
            {
            ...
            case APP_JVMF_PLATFORM:
            {
                AppJVmfPlatform::Finalize(node, appList);
                break;
            }
            case APP_JVMF_SERVER:
            {
                AppJVmfServer::Finalize(node, appList);
                break;
            }
#endif //JVMF_LIB
#ifdef SENSOR_NETWORKS_LIB
