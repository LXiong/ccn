...
enum AppType
{
    ...
    //BFT/JVMF
    //The BFT application must use the Application ID equal to the RPWS
    //listening port for EXata capture to work
    APP_JVMF_SERVER = 8512,
    APP_JVMF_PLATFORM,
...
// takahashi modify
#define APP_TIMER_REGULAR_SEND_PKT (5) /* for sending a regular interest packet*/
#define APP_TIMER_DATA_SEND_PKT (6) /* for sending a fake interest packet*/
#define APP_TIMER_fakeINTEREST_SEND_PKT (7) /* for sending a fake interest packet*/
#define APP_TIMER_RE_SEND_PKT (8) /* for sending a fake interest packet*/
...
struct AppTimer
{
    ...
    uint64_t connectionId;         /* which connection this timer is meant for */
