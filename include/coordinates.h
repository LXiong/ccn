...
void COORD_MapCoordinateSystemToType(
    ...
    {
        coordinates->type = MGRS_CARTESIAN;
    }
#endif // MGRS_ADDON
    else {
        assert(0);
...
#ifdef MGRS_ADDON
    if (coordinateSystemType == (int) MGRS)
    {
        return point->common.c1 >= sw->common.c1 &&
            point->common.c1 <= ne->common.c1 &&
            point->common.c2 >= sw->common.c2 &&
            point->common.c2 <= ne->common.c2;
    }
#endif // MGRS_ADDON
...
#ifdef MGRS_ADDON
    else if (coordinateSystemType == (int) MGRS)
    {
        bool xOverlaps = false;
        bool yOverlaps = false;

        if (sw1->common.c1 < sw2->common.c1)
        {
            // region 1 starts left of region 2
            xOverlaps = ne1->common.c1 >= sw2->common.c1;
        }
        else
        {
            // region 2 starts left of region 1
            xOverlaps = ne2->common.c1 >= sw1->common.c1;
        }
        ...
        if (sw1->common.c2 < sw2->common.c2)
        {
            // region 1 starts below region 2
            yOverlaps = ne1->common.c2 >= sw2->common.c2;
        }
        else
        {
            // region 2 starts below region 1
            yOverlaps = ne2->common.c2 >= sw1->common.c2;
        }
